...
from checkers.consts import EM, PAWN_COLOR, KING_COLOR, OPPONENT_COLOR, MAX_TURNS_NO_JUMP, MY_COLORS, RP, RK, BP, BK
import time
...
def is_lonely(board, square):
    row = square[0]
    col = square[1]
    if (row + 1, col + 1) in board and board[(row + 1, col + 1)] != EM:
        return 0
    if (row - 1, col - 1) in board and board[(row - 1, col - 1)] != EM:
        return 0
    if (row + 1, col - 1) in board and board[(row + 1, col - 1)] != EM:
...
class Player(abstract.AbstractPlayer):
    ...
    def selective_deepening_criterion(self, state):
        ...
        # return len(state.calc_capture_moves()) > 0 and state.curr_player == self.color
