...
struct runqueue {
	...
	task_t *curr, *idle;
	prio_array_t *active, *expired, *shorts, *overdue, arrays[4];
...
// struct cs_visualizer our_tester[256];
// int tester_counter = 0;

// int sys_our_test(void) {
// 	struct cs_visualizer curr;
// 	int i;
// 	for(i = 0; i < tester_counter; i++) {
// 		curr = our_tester[i];
// 		printk("Previous:\n\tPID: %d\tPolicy: %d\tPriority: %d\nNext:\n\tPID: %d\tPolicy: %d\tPriority: %d\nTimestamp: %lu\n
// 			-----------------------------------------\n-----------------------------------------\n", 
// 			curr.prev_pid, curr.prev_policy, curr.prev_st_prio, curr.next_pid, curr.next_policy, curr.next_st_prio, curr.time_stamp);
// 	}
// 	return 0;
// }

// int sys_clear_arr(void) {
// 	tester_counter = 0;
// 	return 0;
// }
...
/*OURS*/
//Used only when waking up a forked process
...
static inline void activate_task(task_t *p, runqueue_t *rq)
{
	...
	if(p->policy != SCHED_SHORT) {
		...
		p->prio = p->static_prio;
		prio_array_t *myArray = rq->shorts;
		if(p->is_overdue)
			myArray = rq->overdue;
		enqueue_task(p, myArray);	
	}
	/*NOT OURS*/
	rq->nr_running++;
}
...
/*OURS*/
static int check_all_resched_cases(task_t *p, task_t *curr) {

	// if(p->policy != 0 || curr->policy != 0 || p->is_overdue != 0 || curr->is_overdue != 0) {
	// 	printk("policy of p: %d, overdue: %d ------ policy of curr: %d, overdue curr: %d\n", p->policy ,p->is_overdue, curr->policy, curr->is_overdue);
	// }

	if(p->is_overdue && p->policy == SCHED_SHORT) {
		return 0;
	}
	if(p->policy == SCHED_SHORT) {
		if(curr->is_overdue && curr->policy == SCHED_SHORT) {
			return 1;
		}
		if(curr->policy == SCHED_SHORT) {
			return p->static_prio < curr->static_prio;
		}
	...
	if(p->prio >= MAX_RT_PRIO) {
		if(curr->policy == SCHED_SHORT && curr->is_overdue) {
			return 1;
		}
		if(curr->policy == SCHED_SHORT) {
			return 0;
...
repeat_lock_task:
	...
	if (!p->array) {
		 ...
		 */
		if(check_all_resched_cases(p, rq->curr)) {
...
void wake_up_forked_process(task_t * p)
{
	...
	if(p->policy == SCHED_SHORT && !p->is_overdue) {
		dequeue_task(p, p->array);
		enqueue_short_task(p, rq->shorts);
	}
	/*NOT OURS*/
	rq_unlock(rq);
}
...
void scheduler_tick(int user_tick, int system)
{
	...
	if (p->array != rq->active && p->array != rq->shorts && p->array != rq->overdue) {
		set_tsk_need_resched(p);
		return;
	}
	spin_lock(&rq->lock);
	...
	if(p->policy == SCHED_SHORT && !p->is_overdue) {
		// printk("SHORT: %d\n", p->time_slice);
		//printk("short and not overdue\n");
		if(!--p->time_slice) {
			//printk("short not overdue and no more ts\n");
			dequeue_task(p, rq->shorts);
			set_tsk_need_resched(p);
			p->is_overdue = 1;
			p->prio = MAX_PRIO-1; //because we want all overdue procwesses to be in the same queue inn rq->overdue.
			p->first_time_slice = 0;
			p->time_slice = (p->requested_ts*HZ)/1000;
		...
		if(!--p->time_slice) {
			...
			p->first_time_slice = 0;
			if(p->cooloffs_left) {
				p->requested_ts = p->new_requested;
			}
			p->time_slice = (p->requested_ts*HZ)/1000;
			if(!p->cooloffs_left) {
				enqueue_task(p, rq->overdue);
...
pick_next_task:
	...
	if(array->nr_active) {
		idx = sched_find_first_bit(array->bitmap);
		if(idx < MAX_RT_PRIO) { // it's RT
			goto setnext;
		}
	}
	array = rq->shorts;
	if(array->nr_active) { // Its a short
		idx = sched_find_first_bit(array->bitmap);
		goto setnext;
	}
	array = rq->active;
	if(array->nr_active) { // its an other
		idx = sched_find_first_bit(array->bitmap);
		goto setnext;
	}
	array = rq->overdue; // its an overdue
	idx = sched_find_first_bit(array->bitmap);
	if(rq->active->nr_active || rq->expired->nr_active) {
		printk("NOT GOOD AT ALL\n");
	}
	//printk("INTRESTING\n");
...
setnext:
		...
		if(prev->array != rq->shorts && current->array != rq->overdue && current->array != rq->active && current->array != rq->expired) {
			//printk("NOT IN A QUEUEUEUE\n");
		} else {
			if(prev->policy == SCHED_OTHER && (prev->array == rq->shorts || prev->array == rq->overdue)) {
				printk("NOT IN RIGHT QUQEUEUQ\n");
			}
		}
	// struct cs_visualizer for_test = 
	// 	{prev->pid, prev->policy, prev->static_prio, next->pid, next->policy, next->prio, jiffies};
	// if(tester_counter < 256)
	// 	our_tester[tester_counter++] = for_test;
	/*NOT OURS*/
switch_tasks:
	prefetch(next);
...
int sys_remaining_time(int pid) {
	...
	if(!p->cooloffs_left || (p->is_overdue && p->cooloffs_left == 1)) {
		return 0;
	}
	return p->time_slice;
}
...
int sys_remaining_cooloffs(int pid) {
	...
	int retval;
	if(!p->is_overdue) {
		retval = p->cooloffs_left;
	} else {
		retval = (p->cooloffs_left == 0) ? 0 : p->cooloffs_left-1;
	}
	return retval;
}
/*NOT OURS*/
...
static int setscheduler(pid_t pid, int policy, struct sched_param *param)
{
	...
	if (lp.sched_priority < 0 || lp.sched_priority > MAX_USER_RT_PRIO-1)
		goto out_unlock;
	/*OURS*/
	if (((policy == SCHED_OTHER) != (lp.sched_priority == 0)) && policy != SCHED_SHORT)
		/*NOT OURS*/
		goto out_unlock;
	...
	if ((current->euid != p->euid) && (current->euid != p->uid) &&
		...
		goto out_unlock;
	/*OURS*/
	if(prevPolicy == SCHED_SHORT && (lp.requested_time < 1 || lp.requested_time > 3000 || lp.number_of_cooloffs < 0 || lp.number_of_cooloffs > 5)) {
		retval = -EINVAL;
		goto out_unlock;
	}
	if(prevPolicy != -1 && p->policy == SCHED_SHORT) { //what if prevPolicy == SCHED_SHORT?
		goto out_unlock;
	}
	if(p->policy == SCHED_SHORT) {
		...
		p->new_requested = lp.requested_time;
	}
	...
	if(prevPolicy != -1 || p->policy != SCHED_SHORT) {
		p->requested_ts = lp.requested_time;
	}
	p->new_requested = lp.requested_time;
	if(prevPolicy == SCHED_SHORT) {
		p->is_overdue = 0;
	}
	p->policy = policy;
	if(p->policy == SCHED_SHORT) {
...
asmlinkage long sys_sched_getparam(pid_t pid, struct sched_param *param)
{
	...
	/*OURS*/
	lp.requested_time = p->requested_ts;
	lp.number_of_cooloffs = p->cooloffs_left;
	/*NOT OURS*/
	read_unlock(&tasklist_lock);
...
asmlinkage long sys_sched_yield(void)
{
	...
	/*OURS*/
	if(current->policy == SCHED_SHORT) {
		list_add_tail(&current->run_list, array->queue[current->prio].next);
		goto out_unlock;
	}
	/*NOT OURS*/
...
void __init sched_init(void)
{
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		rq->shorts = rq->arrays + 2;
		rq->overdue = rq->arrays + 3;
		spin_lock_init(&rq->lock);
		INIT_LIST_HEAD(&rq->migration_queue);

		for (j = 0; j < 4; j++) { //its important that itll be 4!!!
			array = rq->arrays + j;
